...
main() {
  ...
  print('Finding smallest deletable folder');
  print(findSmallestDeletableFolder(tree, tree.totalSize, 40000000));
}
...
int findSmallestDeletableFolder(
    Node root, int totalUsedSpace, int desiredSpace) {
  // check if the root node is a folder
  if (root.type != NodeType.Folder) {
    return 0;
  }

  // check if the root node is deletable
  int smallestDeletableFolder = 0;
  if (totalUsedSpace - root.totalSize <= desiredSpace) {
    smallestDeletableFolder = root.totalSize;
  }

  // traverse the children of the root node
  for (Node child in root.children) {
    int childSize =
        findSmallestDeletableFolder(child, totalUsedSpace, desiredSpace);
    if (childSize > 0 &&
        (smallestDeletableFolder == 0 || childSize < smallestDeletableFolder)) {
      smallestDeletableFolder = childSize;
    }
  }
