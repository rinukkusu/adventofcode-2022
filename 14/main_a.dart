...
bool simulateSand(List<List<int>> cave, int shiftKey) {
  ...
  bool canGoDown(int x, int y) => y + 1 < cave.length && cave[y + 1][x] == 0;
  bool canGoLeftAndDown(int x, int y) =>
      x - 1 < 0 ? throw Error() : canGoDown(x - 1, y);
  bool canGoRightAndDown(int x, int y) =>
      x + 1 > cave[0].length ? throw Error() : canGoDown(x + 1, y);
  void checkOutOfBounds(int y) => y == cave.length - 1 ? throw Error() : true;
